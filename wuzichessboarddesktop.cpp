...
WuZiChessBoardDeskTop::WuZiChessBoardDeskTop(QWidget *parent)
{
    m_chessSideLen = 630;
    m_rows = ROWS;
    m_winner = 0;
...
WuZiChessBoardDeskTop::~WuZiChessBoardDeskTop()
{
}

void WuZiChessBoardDeskTop::initChessBoard()
{
    m_bFinished = false;
    m_stopRandom = true;
    m_turn = -1;
...
void WuZiChessBoardDeskTop::drawChessPiece(int x, int y)
{
    QPainter painter(&m_pixmap);
...
void WuZiChessBoardDeskTop::eraseChessPiece(int x, int y)
{
    QPainter painter(&m_pixmap);
...
void WuZiChessBoardDeskTop::mousePressEvent(QMouseEvent *event)
{
    if(m_bFinished)
    {
        return;
...
void WuZiChessBoardDeskTop::randomize()
{
    if(m_bFinished)
    {
        return;
...
void WuZiChessBoardDeskTop::clearAll()
{
    m_mutex.lock();
...
void WuZiChessBoardDeskTop::cancelLast()
{
    m_mutex.lock();
...
bool WuZiChessBoardDeskTop::isFinished()
{
    return m_bFinished;
}

bool WuZiChessBoardDeskTop::checkFinished()
{
    if(m_vecSteps.size()==0)
    {
        m_bFinished = false;
...
void WuZiChessBoardDeskTop::stopRandomize()
{
    m_mutex.lock();
    m_stopRandom = true;
    m_mutex.unlock();
...
RandomizeThread::RandomizeThread(WuZiChessBoardDeskTop* board):QThread(board)
{
    m_pBoard = board;
}
...
void RandomizeThread::run()
{
    ...
    while(true)
    {
        ...
        if(m_pBoard->m_vecSteps.size()>=(WuZiChessBoardDeskTop::ROWS+1)*(WuZiChessBoardDeskTop::ROWS+1))
        {
            m_pBoard->m_mutex.unlock();
            break;
        }

        int x = rand.getRandNum(WuZiChessBoardDeskTop::ROWS+1);
        int y = rand.getRandNum(WuZiChessBoardDeskTop::ROWS+1);
